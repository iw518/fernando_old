...
def water():
    ...
    holelist=ReceiveHoleBasicInf(projectNo,1)
...
def natural_foundation(projectNo):
    if request.method=="GET":
        ...
        for xLayer in layers:
            xlist.append(
                            (
                                "%.2f"%(xLayer.Ps_Fak(depth,water_depth)) if xLayer.Ps_Fak(depth,water_depth)>0 else '-',
                                "%.2f"%(xLayer.Soil_Fak(depth,water_depth)) if xLayer.Soil_Fak(depth,water_depth)>0 else '-',
                                "%d"%(xLayer.Fak(depth,water_depth)) if xLayer.Fak(depth,water_depth)>0 else '-'
                            )
                        )
